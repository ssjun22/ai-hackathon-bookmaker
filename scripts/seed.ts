...
const BOOKS = [
  {
    ...
    author: '전래동화',
    summary:
      '가난한 나그네가 밥집 앞에서 구수한 냄새만 맡았는데, 욕심 많은 주인이 "냄새 맡은 값을 내라"고 소동을 피웁니다. 지혜로운 원님이 나서서 "냄새 값은 돈 소리로 갚으면 된다"는 재치 있는 판결을 내리고, 욕심의 어리석음과 공평한 지혜를 가르쳐 줍니다.',
    coverImageUrl: null,
  },
  {
    id: 'forest',
    ...
    author: '전래동화',
    summary:
      '착한 형은 요정에게 소원을 말하면 무엇이든 만들어 내는 마법 맷돌을 선물 받습니다. 탐욕스러운 선장이 맷돌을 훔쳐 배에서 "소금 나와라!"를 외쳤지만 멈추는 주문을 몰라 소금이 계속 쌓이다 배가 바닷속으로 가라앉습니다. 그래서 지금도 바다가 짜다고 합니다.',
    coverImageUrl: null,
  },
  {
    id: 'rabbit',
    ...
    author: '전래동화',
    summary:
      '마음씨 좋은 농부가 커다란 무를 원님께 정성껏 선물하자 원님은 기뻐하며 말 한 필을 내려줍니다. 이 소식을 들은 욕심쟁이 부자가 비단을 갖다 바쳤더니 원님은 "마침 좋은 것이 생겼소"라며 농부에게 받은 무를 돌려줍니다. 진심 어린 마음이 가장 값진 선물임을 깨닫게 해 주는 이야기입니다.',
    coverImageUrl: null,
  },
  {
    id: 'brave',
    ...
    author: '전래동화',
    summary:
      '좁은 외나무다리에서 소금장수와 기름장수가 마주쳐 서로 먼저 건너겠다고 고집을 부립니다. 지나가던 지혜로운 노인이 "한 사람이 난간에 기대면 둘 다 지나갈 수 있다"고 알려 줍니다. 소금장수가 먼저 양보하자 두 사람은 친한 친구가 되어, 배려와 양보가 모두를 행복하게 만든다는 교훈을 줍니다.',
    coverImageUrl: null,
  },
] satisfies Array<typeof schema.books.$inferInsert>;

// ────────────────────────────────
// AI 사전 생성 6장 데이터 (mockResults.ts에서 이식)
// ────────────────────────────────
const AI_STORY_PAGES: Array<typeof schema.aiStoryPages.$inferInsert> = [
  {
    bookId: 'star',
    storyTitle: '냄새 맡은 값',
    pages: [
...
async function seed() {
  ...
  await db
    ...
    .values(BOOKS)
    .onConflictDoNothing();
  console.log(`  ${BOOKS.length} books inserted (or skipped if exists)`);

  console.log('Seeding ai_story_pages...');
  await db
    .insert(schema.aiStoryPages)
    .values(AI_STORY_PAGES)
    .onConflictDoNothing();
  console.log(`  ${AI_STORY_PAGES.length} ai_story_pages inserted (or skipped if exists)`);
