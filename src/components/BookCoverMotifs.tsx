/**
 * BookCover에서 사용하는 4종 SVG 모티프 컴포넌트.
 * 각 컴포넌트는 <g> 요소를 반환하며, BookCover의 <svg> 내부에 삽입된다.
 */

export function StarMotif({ accent }: { accent: string }) {
  return (
    <g>
      {/* 점선 별자리 */}
...
export function ForestMotif({ accent }: { accent: string }) {
  return (
    <g>
      {/* 하늘 구름 */}
...
export function RabbitMotif({ accent }: { accent: string }) {
  return (
    <g>
      {/* 배경 꽃잎 패턴 */}
...
export function CarrotMotif({ accent }: { accent: string }) {
  return (
    <g>
      {/* 풀밭 */}
